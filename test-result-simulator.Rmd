...
div(style="text-align: center;",
    ...
    div(style="display:inline-block;", numericInput("rate_a", label = NULL, value = 5.0, width = "100px")),
...
div(style="text-align: center;",
    ...
    div(style="display:inline-block;", numericInput("rate_b", label = NULL, value = 5.5, width = "100px")),
    div(style="display:inline-block; padding-left: 5px; font-weight: bold", "%"))
```
...
We can use the traffic volume, test duration, and average order value, along with our conversion rate lift calculations to see what annualized revenue looks like. _**Keep in mind that these only work if we hold every other revenue factor constant (and we still have a range even with that!)**_
...
output$main_plot <- renderPlot({
  ...
  if(input$rate_b < input$rate_a){
    gg <- ggplot(data = data.frame(x = 1, y = 1, 
      ...
      geom_text(size = 10, fontface = "italic", colour = "#0060AF") +
      theme_light() +
...
output$summary_message <- renderUI({
  ...
  if (input$run_test == 0 | input$rate_b < input$rate_a){
    ...
    # Explanatory note
    exp_message <- div(em("The significance test run above was for a one-tailed test with",  
...
output$annual_rev_message <- renderUI({
  ...
  if (input$run_test == 0 | input$rate_b < input$rate_a){
    ...
    if (p_val() >= 0.05){
      ...
      rev_lift_full_message <- div("In this case, since we didn't achieve statistical significance with the",
      ...
      rev_message_actual <- div("The ACTUAL annual revenue lift from running", em("B"), "instead of", em("A"), 
      ...
      rev_message_observed <- div("The annualized revenue lift based on the", 
      ...
      # annualized revenue.
      range_message <- div("The expected annual revenue lift", strong("RANGE"), "(a 95% confidence interval)",
                           "from running", em("B"), 
                           "instead of", em("A"), "would be ", 
      ...
      rev_lift_full_message <- list(rev_message_actual, br(), rev_message_observed, br(),
                                    range_message)
    }
  }
