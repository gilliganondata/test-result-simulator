...
**This section still needs significant work.**
...
div("The", strong("expected annual revenue lift RANGE"), "from running", em("B"), 
  "instead of", em("A"), "would be (this is overly broad currently -- more stats",
  "work to be done):", strong(textOutput("rev_range", inline = TRUE)))
...
# Calculate the 95% confidence interval for the 2-sample test
...
p_val <- reactive({
  ...
  # power.prop.test() throws an error if there is a large enough n and enough of 
  # a difference between p1 and p2. In those cases, the p-value (sig.level) is just
  # very, very low. So, doing some error handling to account for that.
  # See https://stackoverflow.com/questions/33379962/power-t-test-error -- different
  # test, but the same error results w/ uniroot.
  prop_test <- try(power.prop.test(n = input$num_observations, p1 = a_observed(), p2 = b_observed(), 
                               sig.level = NULL, power = 0.2,  alternative = "one.sided"),
                   silent = TRUE)
  if(inherits(prop_test, "try-error")){
    p_val <- 0
  } else {
    p_val <- prop_test$sig.level
  }
...
output$summary_message <- renderUI({
  ...
  exp_message <- div(em("The significance test run above was for a one-tailed test with",  
                          "a confidence level of 95% and power of 80%. We're not going to get into",
...
# Recording if the type of result we've seen
