...
div(style="text-align: center;",
    div(style="display:inline-block; padding-right: 5px; font-weight: bold; font-family: Lato", "A:"),
    div(style="display:inline-block; font-family: Lato;", numericInput("rate_a", label = NULL, value = 5.0, width = "100px")),
    div(style="display:inline-block; padding-left: 5px; font-weight: bold; font-family: Lato", "%"))
```

```{r}
# Conversion Rate for B
div(style="text-align: center;",
    div(style="display:inline-block; padding-right: 5px; font-weight: bold; font-family: Lato", "B:"),
    div(style="display:inline-block; font-family: Lato", numericInput("rate_b", label = NULL, value = 5.5, width = "100px")),
    div(style="display:inline-block; padding-left: 5px; font-weight: bold; font-family: Lato", "%"))
```
...
output$main_plot <- renderPlot({
  ...
  if(input$rate_b < input$rate_a){
    gg <- ggplot(data = data.frame(x = 1, y = 1, 
      ...
      geom_text(size = 10, fontface = "italic", family = "Lato", colour = "#0060AF") +
      theme_light() +
    ...
    gg <- if(input$run_test == 0) {
      ...
      gg +
        ...
        geom_label(mapping = aes(x=x, y=y + max_y/18, label = paste0(name, format(round(x * 100, 3), nsmall = 3), "%")),
                   size = 5, data = observed_df, family = "Lato") +
...
output$conv_rate_lift <- renderPlot({
  ...
  if (input$run_test == 0 | input$rate_b < input$rate_a){
    ...
    label_actual <- paste0("Actual Lift:\n",
                           percent(rate_b_percent() - rate_a_percent(), accuracy = 0.001))
    label_observed <- paste0("Observed Lift:\n",
                             percent(b_observed() - a_observed(), accuracy = 0.001),
                             " +/- ",
                             percent((max_lift_proportion() - min_lift_proportion())/2, accuracy = 0.001))
